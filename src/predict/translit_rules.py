...
def translit_morph(mada_sentnece_object,loc_mapdict,exceptional_spellings,logger=rules_logger): #TODO: 1) break up into smaller functions. 2) turn transliterator into class with constructor
    ...
    capschar = '±'
    words = mada_sentnece_object.words # list of words --> raw arabic words
    ...
    return recompose(transliterated_sentence,mode='rom') , diacritized_sentence
...
def main():
    ...
    parser.add_argument('output_arabic_txt', help= 'specify output location for Arabic predictions')
    ...
    if args.mode == 'simple':
        ...
        for sent in mada_sent_objects:
            prediction_sentence , prediction_arabic = translit_morph(sent,locmap,locexceptional)
            predictions.append(prediction_sentence)
            arabic_predictions.append(prediction_arabic)

        predictions = pd.DataFrame(predictions)
        arabic_predictions = pd.DataFrame(arabic_predictions)
    else:
    ...
    arabic_predictions.to_csv(args.output_arabic_txt,sep='\t',index=False,header=False)
